...
mgmt_v03 <- mgmt_v02 %>% 
  dplyr::mutate(management_pyric.herbivory = dplyr::case_when(
    site %in% c("KLT", "RIN", "PAW", "RCH.2007") ~ "burn only",
    site %in% c("GIL", "LTR", "PYW", "STE") ~ "graze and burn",
    site %in% c("235", "KLN", "PYN", "PYS", "RCH.2014", "RIS") ~ "patch-burn graze",
...
mgmt_v05 <- mgmt_v04 %>% 
  ...
  dplyr::mutate(time.since.fire_years = dplyr::case_when(
    ...
    site == "RCH.2007" & year %in% (2000 + c(9, 12)) ~ "0",
    site == "RCH.2007" & year %in% (2000 + c(10, 13)) ~ "1",
    site == "RCH.2007" & year == 2011 ~ "2",
    patch == "RCH.2014-C" & year %in% (2000 + c(15, 18)) ~ "0",
    patch == "RCH.2014-C" & year == 2016 ~ "1",
    patch == "RCH.2014-C" & year %in% (2000 + c(14, 17)) ~ "2",
    patch == "RCH.2014-E" & year == 2016 ~ "0",
    patch == "RCH.2014-E" & year == 2017 ~ "1",
    patch == "RCH.2014-E" & year %in% (2000 + c(14, 18)) ~ "2",
    patch == "RCH.2014-E" & year == 2015 ~ "3",
    patch == "RCH.2014-W" & year %in% (2000 + c(14, 17)) ~ "0",
    patch == "RCH.2014-W" & year %in% (2000 + c(15, 18)) ~ "1",
    patch == "RCH.2014-W" & year == 2016 ~ "2",
